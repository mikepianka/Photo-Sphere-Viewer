...
import { SYSTEM, utils, CONSTANTS, PSVError, EquirectangularAdapter } from 'photo-sphere-viewer';
...
function createBaseTexture(img, blur, getHeight) {
  if (blur || img.width > SYSTEM.maxTextureWidth) {
    var ratio = Math.min(1, SYSTEM.getMaxCanvasWidth() / img.width);
...
var EquirectangularTilesAdapter = /*#__PURE__*/function (_EquirectangularAdapt) {
  ...
  function EquirectangularTilesAdapter(psv, options) {
    ...
    if (!utils.isPowerOfTwo(_this.config.resolution)) {
    ...
    _this.psv.on(CONSTANTS.EVENTS.POSITION_UPDATED, _assertThisInitialized(_this));

    _this.psv.on(CONSTANTS.EVENTS.ZOOM_UPDATED, _assertThisInitialized(_this));
  ...
  _proto.createMesh = function createMesh(scale) {
    ...
    var geometry = new THREE.SphereGeometry(CONSTANTS.SPHERE_RADIUS * scale, this.SPHERE_SEGMENTS, this.SPHERE_HORIZONTAL_SEGMENTS, -Math.PI / 2).scale(-1, 1, 1).toNonIndexed();
...
export { EquirectangularTilesAdapter };
