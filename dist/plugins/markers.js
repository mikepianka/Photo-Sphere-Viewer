...
import { utils, CONSTANTS, PSVError as PSVError$1, AbstractButton, DEFAULTS, registerButton, AbstractPlugin } from 'photo-sphere-viewer';
import 'uevent';
...
function greatArcIntermediaryPoint(p1, p2, f) {
  ...
  var λ2 = p2[0],
      φ2 = p2[1];
  var r = utils.greatArcDistance(p1, p2);
...
function getPolylineCenter(polyline) {
  ...
  for (var i = 0; i < polyline.length - 1; i++) {
    var l = utils.greatArcDistance(polyline[i], polyline[i + 1]) * CONSTANTS.SPHERE_RADIUS;
    lengths.push(l);
    length += l;
...
var Marker = /*#__PURE__*/function () {
  ...
  function Marker(properties, psv) {
    if (!properties.id) {
      throw new PSVError$1('missing marker id');
    }
    /**
     * @member {PSV.Viewer}
     * @readonly
  ...
  _proto.getScale = function getScale(zoomLevel, position) {
    ...
    if (Array.isArray(this.config.scale.longitude)) {
      ...
      scale *= _bounds[1] + (_bounds[0] - _bounds[1]) * CONSTANTS.EASINGS.outQuad(Math.max(0, (halfFov - arc) / halfFov));
    }
  ...
  _proto.showTooltip = function showTooltip(mousePosition) {
    if (this.props.visible && this.config.tooltip.content && this.props.position2D) {
      ...
      if (this.isPoly()) {
        var viewerPos = utils.getPosition(this.psv.container);
        config.box = {
  ...
  _proto.update = function update(properties) {
    ...
    if (newType !== undefined && newType !== this.type) {
      throw new PSVError$1('cannot change marker type');
    }
    ...
    this.props.anchor = utils.parsePosition(this.config.anchor); // clean scale
  ...
  _proto.__updateNormal = function __updateNormal() {
    if (!utils.isExtendedPosition(this.config)) {
      throw new PSVError$1('missing marker position, latitude/longitude or x/y');
    }

    if (this.config.image && (!this.config.width || !this.config.height)) {
      throw new PSVError$1('missing marker width/height');
    }
  ...
  _proto.__updateSvg = function __updateSvg() {
    ...
    utils.each(this.props.def, function (value, prop) {
      _this.$el.setAttributeNS(null, prop, value);
    }); // set style

    if (this.config.svgStyle) {
      utils.each(this.config.svgStyle, function (value, prop) {
        _this.$el.setAttributeNS(null, utils.dasherize(prop), value);
      });
    } else {
  ...
  _proto.__updatePoly = function __updatePoly() {
    ...
    if (this.config.svgStyle) {
      utils.each(this.config.svgStyle, function (value, prop) {
        _this2.$el.setAttributeNS(null, utils.dasherize(prop), value);
      });
  ...
  Marker.getType = function getType(properties, allowNone) {
    ...
    var found = [];
    utils.each(MARKER_TYPES, function (type) {
      if (properties[type]) {
        found.push(type);
      }
    });
    ...
    if (found.length === 0 && !allowNone) {
      throw new PSVError$1("missing marker content, either " + Object.keys(MARKER_TYPES).join(', '));
    } else if (found.length > 1) {
      throw new PSVError$1("multiple marker content, either " + Object.keys(MARKER_TYPES).join(', '));
    }
...
var MarkersPlugin = /*#__PURE__*/function (_AbstractPlugin) {
  ...
  function MarkersPlugin(psv, options) {
    ...
    if ((options == null ? void 0 : options.listButton) === false || (options == null ? void 0 : options.hideButton) === false) {
      utils.logWarn('MarkersPlugin: listButton and hideButton options are deprecated. ' + 'Please define the global navbar options according to your needs.');
    }
    /**
     * @member {HTMLElement}
     * @readonly
  ...
  _proto.init = function init() {
    ...
    if (this.config.markers) {
      this.psv.once(CONSTANTS.EVENTS.READY, function () {
  ...
  _proto.handleEvent = function handleEvent(e) {
    ...
    switch (e.type) {
      ...
      case CONSTANTS.EVENTS.DOUBLE_CLICK:
  ...
  _proto.showAllTooltips = function showAllTooltips() {
    ...
    utils.each(this.markers, function (marker) {
  ...
  _proto.addMarker = function addMarker(properties, render) {
    ...
    if (this.markers[properties.id]) {
      throw new PSVError$1("marker \"" + properties.id + "\" already exists");
    }
  ...
  _proto.getMarker = function getMarker(markerId) {
    ...
    if (!this.markers[id]) {
      throw new PSVError$1("cannot find marker \"" + id + "\"");
    }
  ...
  _proto.showMarkersList = function showMarkersList() {
    ...
    this.psv.panel.show({
      ...
      content: MARKERS_LIST_TEMPLATE(markers, this.psv.config.lang[MarkersButton.id], utils.dasherize(MARKER_DATA)),
      noMargin: true,
  ...
  _proto.__getTargetMarker = function __getTargetMarker(target, closest) {
    ...
    var target2 = closest ? utils.getClosest(target, '.psv-marker') : target;
