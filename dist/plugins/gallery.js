...
*/
import { utils, AbstractButton, SYSTEM, AbstractComponent, DEFAULTS, registerButton, CONSTANTS, PSVError, AbstractPlugin } from 'photo-sphere-viewer';
...
var GalleryComponent = /*#__PURE__*/function (_AbstractComponent) {
  ...
  function GalleryComponent(plugin) {
    ...
    _this.container.addEventListener(SYSTEM.mouseWheelEvent, _assertThisInitialized(_this));
  ...
  _proto.handleEvent = function handleEvent(e) {
    ...
    switch (e.type) {
      case SYSTEM.mouseWheelEvent:
        var spinY = utils.normalizeWheel(e).spinY;
      ...
      case 'click':
        ...
        if (Math.abs(this.prop.initMouseX - e.clientX) < 10) {
          var item = utils.getClosest(e.target, "[data-" + GALLERY_ITEM_DATA_KEY + "]");
    ...
    switch (e.type) {
      case CONSTANTS.EVENTS.PANORAMA_LOADED:
        var item = this.items.find(function (i) {
          return utils.deepEqual(i.panorama, e.args[0].panorama);
        });
