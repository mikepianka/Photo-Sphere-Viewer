...
import { CONSTANTS, utils, AbstractPlugin } from 'photo-sphere-viewer';
  ...
  _proto.setLatitudeRange = function setLatitudeRange(range) {
    ...
    if (range) {
      this.config.latitudeRange = range.map(function (angle) {
        return utils.parseAngle(angle, true);
      });
    } else {
